...
class GradCAM:
    ...
    def forward(self, input, class_idx=None, retain_graph=False, trt_engine = None):
        b, c, h, w = input.size()
   
        activations, logit, score, gradients = [torch.from_numpy(t).cuda() for t in trt_out]
    ...
    def __call__(self, input, class_idx=None, retain_graph=False, trt_out = None):
        return self.forward(input, class_idx, retain_graph, trt_out)
...
class GradCAMpp(GradCAM):
    ...
    def forward(self, input, class_idx=None, retain_graph=False, trt_out = None):
        b, c, h, w = input.size()
   
        activations, logit, score, gradients = [torch.from_numpy(t).cuda() for t in trt_out]
        #logit, activations, score, gradients = self.model_arch(input)

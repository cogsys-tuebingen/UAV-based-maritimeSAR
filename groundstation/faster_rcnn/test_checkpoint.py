...
from faster_rcnn.train import FasterRCNNModule, ResizerRectangle, Normalizer, collater, FitIntoGrid
...
def load_checkpoint(path):
    model = FasterRCNNModule.load_from_checkpoint(path)
    return model
...
def predict_whole_valset(model, val_generator, device):
    ...
    for sample in tqdm.tqdm(val_generator, desc='Predict validation set (whole)'):
        ...
        for img in sample['img']:
            detection = model([img.float().to(device)])
            if len(detection)>0:
                detection = detection[0]
                s, l, b = detection['scores'], detection['labels'], detection['boxes']
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        network = model.hparams['network'] if 'network' in model.hparams.keys() else 'Faster-RCNN'
        backbone = model.hparams['backbone'] if 'backbone' in model.hparams.keys() else '?'

        random_cropping = model.hparams['random_cropping'] if 'random_cropping' in model.hparams.keys() else False

        print_result(network, backbone, model.hparams['image_size'], coco_stats, tide_stats, random_cropping)

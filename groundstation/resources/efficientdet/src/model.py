...
from efficientdet.src.utils import BBoxTransform, ClipBoxes, Anchors
...
class EfficientDet(nn.Module):
    def __init__(self, hparams, anchors, num_anchors=9, num_classes=20, compound_coef=0,
                 efficientnet_model='efficientnet-b0'):
        super(EfficientDet, self).__init__()
        self.hparams = hparams
        self.compound_coef = compound_coef
        self.score_threshold = self.hparams['score_threshold']
        self.efficientnet_model = efficientnet_model

        self.num_channels = [64, 88, 112, 160, 224, 288, 384, 384][self.compound_coef]

        self.conv3 = nn.Conv2d(EfficientDetBackendLayerSize[efficientnet_model][0], self.num_channels, kernel_size=1, stride=1, padding=0)
        self.conv4 = nn.Conv2d(EfficientDetBackendLayerSize[efficientnet_model][1], self.num_channels, kernel_size=1, stride=1, padding=0)
        self.conv5 = nn.Conv2d(EfficientDetBackendLayerSize[efficientnet_model][2], self.num_channels, kernel_size=1, stride=1, padding=0)
        self.conv6 = nn.Conv2d(EfficientDetBackendLayerSize[efficientnet_model][3], self.num_channels, kernel_size=3, stride=2, padding=1)
        ...
        self.backbone_net = EfficientNet(efficientnet_model)
